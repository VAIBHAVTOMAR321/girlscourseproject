...
const API_URL = 'https://brjobsedu.com/girls_course/girls_course_backend/api/employee-issues/'
...
const EmployeeIssues = () => {
    ...
    if (searchTerm !== '') {
      filtered = filtered.filter(query => {
        return query.full_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
          query.unique_id.toLowerCase().includes(searchTerm.toLowerCase()) || // Changed from student_id
  ...
  const submitReply = async () => {
    try {
      ...
      fetchQueries()
      alert('Employee issue updated successfully!')
    } catch (error) {
      console.error('Error updating employee issue:', error)
      alert('Failed to update employee issue')
    }
  }
  ...
  const confirmDelete = async () => {
    try {
      ...
      fetchQueries()
      alert('Employee issue deleted successfully!')
    } catch (error) {
      console.error('Error deleting employee issue:', error)
      alert('Failed to delete employee issue')
    }
  }
  ...
  return (
    <div className="admin-layout">
            ...
            <Container fluid className='mob-top-view'>
              <div className="d-flex justify-content-between align-items-center mb-4 page-header">
                <div className="d-flex align-items-center all-en-box gap-3">
                  ...
                  <h4 className="mb-0">Employee Issues / Queries</h4>
                </div>
              </div>
                        ...
                        <Table hover className="custom-table align-middle mb-0">
                          <thead className="table-light custom-table">
                            <tr>
                              ...
                              <th>Employee Name</th>
                              <th>Employee ID</th> {/* Changed from Student ID */}
                              <th>Title</th>
                              <th>Issue</th>
                              <th>Status</th>
                              <th>Extra Remark</th>
                            ...
                            {currentRecords.length === 0 ? (
                              ...
                              currentRecords.map((query) => (
                                <tr key={query.id}>
                                  ...
                                  <td className="small">{query.unique_id}</td> {/* Changed from student_id */}
                              ...
                              <Card.Body className="p-3">
                                <div className="d-flex justify-content-between align-items-start mb-2">
                                  <div>
                                    ...
                                    <small className="text-muted">ID: {query.unique_id}</small> {/* Changed from student_id */}
                                  </div>
      ...
      <Modal show={showReplyModal} onHide={() => setShowReplyModal(false)} centered size="lg">
        <Modal.Header closeButton className="border-bottom py-2 px-3">
          <Modal.Title className="fw-semibold fs-6">
            Reply to Issue - {selectedQuery?.query_id}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body className="">
          {selectedQuery && (
            <div>
              <Row className="mb-3">
                <Col md={6} xs={12}>
                  <p className="mb-1"><strong>Employee Name:</strong></p> {/* Changed from Student Name */}
                  <p className="text-muted">{selectedQuery.full_name}</p>
                </Col>
                <Col md={6} xs={12}>
                  <p className="mb-1"><strong>Employee ID:</strong></p> {/* Changed from Student ID */}
                  <p className="text-muted">{selectedQuery.unique_id}</p> {/* Changed from student_id */}
                </Col>
              </Row>
              <Row className="mb-3">
                <Col md={12}>
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)} centered>
        ...
        <Modal.Body className="">
          <p>Are you sure you want to delete this employee issue?</p>
